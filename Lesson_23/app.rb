...
class App
  ...
  def types_carriage(number, type)
    return passenger_carriage(number) if type == 1
    return cargo_carriage(number) if type == 2
  end

  def passenger_carriage(number)
    puts 'How many passenger seats?'
    passenger_seats = gets.chomp.to_i
    carriages << PassengerCarriage.new(number, passenger_seats)
  end

  def cargo_carriage(number)
    puts 'What cargo volume?'
    cargo_volume = gets.chomp.to_i
    carriages << CargoCarriage.new(number, cargo_volume)
  end
